...
Kotlin::
In build.gradle.kts, to specify version 0.19.a:
[source,kotlin,linenumbers]
----
qalipsis {
    version("0.18.e")
...
----
Groovy::
In build.gradle, to specify version 0.18.e:
[source,kotlin,linenumbers]
----
qalipsis {
    version = '0.18.e'
...
To specify version 0.18.e, add the version in the gradle.properties or Gradle execution options:
[source,kotlin,linenumbers]
----
qalipsis.version=0.18.e
----
...
All tasks extending `RunQalipsis` write the plain-text and HTML reports to the `build/reports/qalipsis` folder, and the JUnit test results to the `build/test-results/qalipsis` folder. This behavior can be disabled or overridden through configuration, as shown below.
...
tasks {
    named("qalipsisRunAllScenarios") {
        configuration(
            "report.export.html.enabled" to "true",
            "report.export.html.folder" to project.layout.buildDirectory.dir("qalipsis/reports").get().asFile.path
        )
    }
}
----
...
tasks.named('qalipsisRunAllScenarios') {
    configuration 'report.export.html.enabled': 'true',
                  'report.export.html.folder': layout.buildDirectory.dir('qalipsis/reports').get().asFile.path
}
----
=== Custom Execution
...
tasks {
    create("executeRestApiSpikeTest", RunQalipsis::class.java) {
        ...
        configuration("report.export.html.enabled" to "true",
            "report.export.html.folder" to project.layout.buildDirectory.dir("qalipsis/reports").get().asFile.path
        )
    }
}

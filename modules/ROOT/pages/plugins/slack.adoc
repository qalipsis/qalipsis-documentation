...
|`enabled`
a|Activates/deactivates campaign report publishing to Slack; must be set to true to enable publishing to Slack.
...
|`token`
a|The bot token of the app in charge of posting messages to the specified workspace channel, usually beginning with `xoxb`. .
 *Optional/Required:* Required +
 *Data Type:* String + 
 *Default Value:* N/A + 
...
|`channel`
a|A valid Slack channel name within a workspace indicating where notifications messages should be pushed to. +
 *Optional/Required:* Required +
 *Data Type:* String + 
 *Default Value:* false + 
...
|`status`
a|Specifies which campaign report statuses should trigger a notification. +

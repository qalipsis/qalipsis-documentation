...
... A `verify` step that asserts that the server's response is matches the expectations and to verify the server's responsiveness.
... An `HTTP WITH` step that reuses the HTTP connection to send further requests.
... A second `assertion` step to verify the server's responses and responsiveness.
...
== Create a project
Start by creating a new project using the xref:get-started/bootstrap.adoc[QALIPSIS bootstrap project].

. Configure the project to use the plugins you need for your 
scenario:
+
[source,kotlin,linenumbers]
----
...
* The reason for the failure: `PT0.10696475S should be < PT0.1S` (the durations are displayed in the ISO-8601 format).

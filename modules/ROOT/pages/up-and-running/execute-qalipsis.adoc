...
[source,shell]
----
java -jar my-scenario-qalipsis.jar <command-line-arguments>
----

== Start QALIPSIS from the Docker image [.badge.oss]## ##[.badge.commercial]## ##

If you prefered to package your QALIPSIS project as a Docker image, you can start QALIPSIS using the following command, where <command-line-arguments> are the arguments you want to pass to QALIPSIS
...
If you need to explicitely specify the role of each node in the cluster, this can be done using the `head` or `factory` options as command line when starting QALIPSIS.

[example,shell]
----
java -jar my-scenario-qalipsis.jar head <command-line-arguments>

docker run --rm -it -v $(pwd):/opt/qalipsis/config my-scenario:1.0 head <command-line-arguments>
----

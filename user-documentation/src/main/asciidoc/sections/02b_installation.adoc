...
However, the ease of build and lightness of an ephermeral installation makes it a very portable alternative for verifying test systems and performing demos. 
...
* In your QALIPSIS project, execute the task `package-ephemeral` which will assemble your project and create a ZIP file  `build/install/your-project-name-your-version-qalipsis.jar`.
+ 
...
=== Configuring a transient instalation for project deployment
...
* Add the ZIP archive to your installation folder of choice and uncompress it: `unzip qalipsis-your-project-name.zip` or use another compression tool of your choice. 
...
* Add the ZIP archive to your installation folder of choice and uncompress it: `unzip qalipsis-your-project-name.zip`. 
...
* Ceate a `qalipsis.yml` file to configure access to the database for the master data:
...
=== Light cluster
A light cluster requires minimal effort for installation; however, it is limited in terms of load capabilitiy.
...
As a prerequesite, you need to install QALIPSIS as described in the <<Transient installations,`transient`>>, <<Standalone Installations,`standalone`>> or <<Cluster Installations,`cluster`>> installation sections. 
...
. Uncompress the file: `unzip qalipsis-scenario-your-project-name.zip` or by using another compression tool of your choice.
...
:warning: WARNING
Different scenarios might require different dependencies; pay attention to the conflict of versions when you overwrite existing files inan existing installation.
...
You can add your own scenarios and dependencies as prepared for an <<Amendable installations,Amendable installation>> with the following Dockerfile:

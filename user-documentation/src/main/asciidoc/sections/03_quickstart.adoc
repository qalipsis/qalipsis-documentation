...
The sections will guide you through the creation of your first "Hello World" scenario.

NOTE: Before getting started, confirm that you have Java 11 (or above) SDK installed on your workstation.

While it is possible to develop a complete scenario with a text editor, we encourage you to use a suitable IDE such as IntelliJ or Eclipse.

=== Create a project with Gradle - Kotlin DSL

The first step is to create a new project for Gradle with your IDE, using the Kotlin DSL.
----
...
Furthermore, QALIPSIS relies on the Micronaut IoC framework, which requires the following setup???

==== IntelliJ

Jollow the instructions provided by Micronaut's team in https://docs.micronaut.io/latest/guide/index.html#ideaSetup[section for IntelliJ].

...
By nature, a plugin is an optional component of the tool. It allows you to include only what you need while ignoring components and libraries that do not serve your your present concerns.
...
Default implementations of the above functionalities are part of the QALIPSIS core. However, the default implementations may not be suited for productive testing or constant monitoring. Plugins enhance the above functionalities by providing related implementations.

== Plugin Integration
To create a test scenario, a tiny Gradle project is required. The plugin is simply added as a dependency to the class path of the project. You can read more about it in <<Creating a Scenario>> within <<Quickstart>>.

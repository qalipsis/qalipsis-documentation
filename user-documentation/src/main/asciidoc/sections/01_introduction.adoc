...
(Plugins, the cloud-based application, and other premium extensions are not included in the open source vesion.)
...
The QALIPSIS Graphical User Interface (GUI) allows users to drive straightforward test runs: start and stop, change the load distribution and increase or decrease the number of simulated users.
...
* Cross-verify data from different sources: send requests and then 
check responses, analyze infrastructure metrics, process middleware events, and view  database records and response time in a single operation.

...
QALIPSIS is an enterprise-grade load and performance-testing tool especially designed for distributed systems.
...
+
Commercial-licensed-based plugins, the cloud-based application, and other premium extensions are not included in the free version.
...
The QALIPSIS Graphical User Interface (GUI) allows users to drive straight forward test runs: start and stop, configure the load distribution, and increase or decrease the number of simulated users. The GUI also allows the user to view reports, schedule a test, etc.

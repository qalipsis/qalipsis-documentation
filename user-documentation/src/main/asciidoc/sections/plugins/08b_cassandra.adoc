...
The example scenarios in this section demonstrate how to save, poll and search data using the https://cassandra.apache.org/_/index.html[cassandra] plugin.

==== cassandra-save-and-search
The example `cassandra-save-and-search` scenario saves records to a database using the `save` step, queries a specific location in the database using the `search` step, and then verifies that the returned data matches the data in the original saved record.
...
<8> Define the list of columns in the database.
<9> Define the list of rows to save to the database.

==== cassandra-save-and-poll 
The example `cassandra-save-and-poll` scenario saves records to a database using the `save` step, periodically retrieves records from the database using the `poll` step, and then verifies that the retrieved records match the original saved records.

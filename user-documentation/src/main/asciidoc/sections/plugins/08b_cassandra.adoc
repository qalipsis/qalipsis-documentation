...
* <<SaveandAndSearch step,`saveandsearch>>
...
=== Cassandra-save-and-search step
The `cassandra-save-and-search`  ......

Ancestor::
 Scenario

Functionality::
 The `cassandra-save-and-search` step's .....
...
** `keyspace`(required): the outermost object that determines how data replicates on nodes.
** `datacenterProfile`(required): an execution profile that encapsulates a group of settings associated with individual queries.
* `monitoring`: configures the monitoring parameters for the step.
...
=== Cassandra-save-and-poll step
The `cassandra-save-and-poll` step  ......

Ancestor::
 Scenario

Functionality::
 The `cassandra-save-and-poll` step's .....
...
Notable parameters::
* classpath(file): reads the data from a specified class path resource; file ("battery-levels.csv") is the path to the resource with the leading slash ignored.
* BatteryState:
* `connect` (required): configures the connection to https://cassandra.apache.org[APACHE Cassandra]
** `keyspace`(required): the outermost object that determines how data replicates on nodes.
** `datacenterProfile`(required): an execution profile that encapsulates a group of settings associated with individual queries.

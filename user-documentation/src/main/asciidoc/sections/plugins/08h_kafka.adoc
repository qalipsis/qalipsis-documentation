...
* `bootstrap` (optional): bootstraps (string of host and port pair) of the Kafka broker; defaults to `localhost:9092`.
* `topic` (optional): string name of the topic to write the events to; defaults to `qalipsis-events`.
* `duration-as-nano` (optional): boolean flag that activates/deactivates converting the duration to nanoseconds; defaults to `false` (converts to milliseconds).
* `linger-period` (optional): maximum period between two publications of events to Kafka; positive duration; defaults to `10s`.
* `batch-size` (optional): maximum number of events buffered between two publications of events to Kafka; positive integer; defaults to `2`.
...
* `bootstrap.servers` (optional): bootstraps (string of host and port pair) of the Kafka broker; defaults to `localhost:9092`.
* `topic` (optional): string name of the topic to write events to; defaults to `qalipsis-meters`.
* `linger.ms` (optional): maximum period between two batches of events; positive duration; defaults to `5000`.
* `batch-size` (optional): maximum number of events buffered between two publications of events to Kafka; positive integer; defaults to `800000` (in bytes).
* `configuration`: key, value configuration parameters of type `Properties`, for the Kafka producer.
* `delivery.timeout.ms` (optional): maximum integer value in milliseconds before time out.; defaults to `10000`.

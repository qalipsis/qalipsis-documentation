...
+
** `ALL` triggers notification for any of the campaign report statuses.
+
** A combination of `ABORTED` and `FAILED` statuses only triggers notification when the campaign report status is either ABORTED OR FAILED. 
+
** Selecting just one value (e.g., `SUCCESSFUL`) triggers notifications only when the campaign status is `SUCCESSFUL`.
...
After installation, a **QALIPSIS** bot token will display under **Apps** in the left sidebar of your Slack workspace.  

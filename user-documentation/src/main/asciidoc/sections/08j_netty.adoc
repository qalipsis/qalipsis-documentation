...
Client Library::
Netty: refer to https://netty.io/[Netty].

Supported Steps::
* <<HTTP Step,HTTP>>: client for datatracker.ietf.org/doc/rfc2616[HTTP 1.1] and https://datatracker.ietf.org/doc/rfc7540/[HTTP 2.0].
...
* <<UDP Step,`UDP`>>: client to exchange data with a UDP endpoint.
...
+
* When the step is kept open without limit, a <<CLOSE TCP Step,`CLOSE TCP`>> step must be executed to properly close the connections and avoid a saturation of the system.
+
...
Notable Parameters::
This step only accepts the name of an `HTTP` step owning an `HTTP` connection as an argument. 
...
Notable Parameter::
The `CLOSE TCP` step only accepts the name of a previous `HTTP` step as an argument.
 
== UDP Step 
The `UDP` step within the `netty` plugin is a client for UDP.

It is best practice to use `UDP` step when reusing a `TCP` connection across several steps without a pool of connections.

Ancestors:
Scenario and Step

Functionality::
The `UDP` step creates a UDP Datagram packet from the `request` and sends it to the remote endpoint which is waiting for a response.  

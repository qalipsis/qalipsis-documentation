...
== Definitions 
In QALIPSIS, each step generates different kinds of metadata that provides insight on the overall behvior and performances of the system.
...
Scenarios within campaighns are made up of steps.  By default, monitoring is disabled for each step.
Note that not all steps are conducive monitoring. For example, there would be nothing to monitor in a delay step.
However, load genaerating and assertion steps genarate data that is important to monitor.
...
Plugin steps must be also configured to export monitoring data. As at the time of this writing, the following plugins are supported:
...
* Kafka: Refer to the link:plugins/08h_kafka.adoc[kafka plugin]  section for details.  
* Timescaledb: Refer to the link:plugins/08d_timescaledb.adoc[Timescaledb plugin] section for details.
* Influxdb: Refer to the link:plugins/08e_Influxdb.adoc[influxdb plugin]  section for details.


NOTE: It is important to balance the count of publishers (when configurable) with amount of event data that steps generate.  If the amount of data is too large for the publishers to keep up with there may be unwanted side-effects. For example, the events buffer size may increase enough to cause memory issues and/or QALIPSIS may  need time to exit after the end of a campaign to empty the buffers.

=== Exporting specifc event data 

...
----
<1> All 100 minions are deployed simultaneously. 
<2> The `.tcp` step ("my-tcp") is the root of the scenario.
<3> The `.reuseTCP` step ("reuse-tcp") is a successor of the previous step ("my-tcp") and receives the previous step's output as its input. T
...
* `delay` adds a constant delay before executing the next step, keeping the same execution pace as previous step.In the example, the `delay` between steps is set to `123` MS.
+
[source,kotlin,linenumbers]
----
...
* `unshelve` extracts a specific value from each map in a collection of maps and creates a new collection with those values. In the example, `unshelve<Int, Double>("value-1")` takes two type arguments `<Int, Double>`, which specify the types of the key and value in the map respectively, and a single argument `"value-1"`, which is the key of the value to extract from each map.
...
* `pipe` simply forwards the input to the output. In the example, `.pipe`prints the result of the previous step and pass it along to the next step.
...
* `blackHole` consumes the input and provides no output. In the example, `.filter` filters out the even numbers and `.blackHole` discards them.
+
[source,kotlin,linenumbers]
----
...
In the example, `.collect` collects the elements of a flow with a timeout of `1` second and a concurrency level of `1`.

[source,kotlin,linenumbers]
----
.collect(Duration.ofSeconds(1), 1)
----

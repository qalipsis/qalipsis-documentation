...
Any number of minions can act on a scenario. The minion count for a scenario is set during scenario configuration (refer to link:04_scenario.adoc[Scenario Configuration] or at runtime (refer to link:06_configuration.adoc[QALIPSIS Configuration]).  
...
Refer to the link:05_steps.adoc[Step Specifications] section of this manual for details.  
...
=== Head
The head node of a QALIPSIS cluster is responsible for orchestration and reporting.

=== Factory
The factory node of a QALIPSIS cluster hosts the minions that will execute the load tests and assertions. 

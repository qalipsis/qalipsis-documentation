...
Any number of minions can act on a scenario. The minion count for a scenario is set during scenario configuration (refer to <<Scenario specification>> or at runtime (refer to <<QALIPSIS Configuration>>).
...
Refer to the <<Steps>> section of this manual for details.

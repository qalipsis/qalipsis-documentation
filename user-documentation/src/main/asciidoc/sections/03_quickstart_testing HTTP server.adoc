...
Note that this Quickstart Guide assumes that:

* You have a general understanding of the <<QALIPSIS Core concepts>>.
* You have Java 11 or above SDK installed on your workstation.

== What You Will Learn
This guide will walk you through steps to:

. Start an HTTP server.
. Create a skeleton project. 
. Execute plain HTTP queries with Qalipsis as a standalone application.
. Assert responses from the server.
. Reuse an HTTP connection for further calls.
. Create a Java archive to share your scenario.
. Visualize the events within your scenario via a log file.

== Start the HTTP server

 Start the HTTP service using the docker-compose.yml file and by running the command docker compose up -d when located in its parent directory.
## Create A project

1.In your browser, open the website [https://launch.qalipsis.io](https://launch.qalipsis.io).

1. . How to create and configure a scenario.
Verify the prerequisites listed on the page.
...
* Learn how to push log file events to a database. <<where is this documented>> 
* Work through other QALIPSIS  Quickstart Guides:
** quickstart 1
** quickstart 2

other QALIPSIS Quickstart Guides to learn how to Set up  other ways This quickstart ends here, and of course, log files are not the best to work with a massive amount of events, whether they are failures or not.

In the QALIPSIS, you will discover how to push those events to databases, so that you can query and report them in a fancier way.
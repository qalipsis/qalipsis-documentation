...
In order to provide the maximum flexibility to configure a scenario and the best performance while executing it, QALIPSIS splits its configuration from its execution.
...
QALIPSIS provides some operators for common use cases, and plugins bring others to extend the scope of the features you can use, such as `scenario`, `configure`, `map`, `delay` and others. 

You will discover a certain number of operators in the next sections, but the list is actually infinite, because you can create your own.
See <<Create your own plugin>> for more details.
Refer to <<Qalipsis Operators>>.

==== Creating a scenario

The first step in creating a scenario is to call the scenario function which initiates a new scenario specification with the provided name:

[source,kotlin,linenumbers]
----
@Scenario
fun myScenario() {
    val myScenario = scenario("hello-world") { //Scenario defaualt 
    }
    .start() // <3>
    .returns { // <4>
...
----
<1> Call to the scenario function, which initiates a new scenario specification with the provided name.
...
NOTE: While <<Load-injected trees>> will be visited by the number of minions you specified, the others will get only one.

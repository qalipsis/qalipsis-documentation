...
minionsCount = A scenario specification is made up of several parts.
...
.start()
.returns {//add step specifications here.}
----
...
+
* When calling the first iteration of a user-defined strategy, the initial value of `pastPeriodMs` is 0. However, you can use your own variable to verify if this is the first iteration or not.
...
Once all attempts are executed per the defined `retryPolicy`, the step execution context is marked as exhausted and only <<Error processing steps>> are executed.
...
----
NOTE that there is no provided `RetryPolicy` in the current version of QALIPSIS; however, you can create your own implementation to match your needs.
+
...
_myScenario_ identifies a second graph visited by a unique minion and generally consisting of iterative operations such as polling of datasources.
...
To create two load-injected branches, you can use the `.tube()` and `.split` commands.

* `.tube()`  is non-operation step that simply forwards the its input to its  output.
...
In the following example, two branches are diverging: one with the steps `execute{}.map{}` and a second with `flatten().filterNotNull()`. Both run concurrently after each execution of _tube()_.
...
Each minion receives the next unused record emitted from the step. Once there are no more records to provide, all the  minions remain in the step.

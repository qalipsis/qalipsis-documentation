...
=== Execution profile / ramping strategies
* Mandatory
* There is no default
...
* `delay`: initial delay from a failure to the next attempt; type = Duration; defaults is `1 second`.
* `multiplier`: factor to multiply delay between each attempt; type = Double; defaults is `1.0`.
* `maxDelay`: maximal duration allowed between two attempts; type = Duration; default is `1 hour`.

Example::
In the example, there are 10 retries after a step failure.
...
* The 6th through 10th retries occurs 10 ms after the previous retry.
+
...
<4> Sets the multiplier to `2.

Once all attempts are executed per the defined `BackoffRetryPolicy`, the step execution context is marked as exhausted and only error processing steps are executed.  Refer to <<Step specifications>> for more information on error processing steps.

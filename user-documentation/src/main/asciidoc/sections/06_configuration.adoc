...
You can overwrite the default configuration and add your own properties by creating a file called `qalipsis-config.yml` either in the class path or in the working directory or both. Both the default configuration and the  `qalipsis-config.yml` file can specify the same parameters; however, the `qalipsis-config.yml` has precedence.
